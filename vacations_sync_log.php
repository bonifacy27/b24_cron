...
SELECT TOP ".GATE_BATCH_SIZE."
  Absence_ID,Staff_ID,Absence_Status,Absence_Renew_Date,Absence_State,Absence_Date_Start
...
ORDER BY Absence_Renew_Date ASC, Absence_ID ASC";
    ...
    foreach ($rows as $row) {
        ...
        $sqlRenew = $row['Absence_Renew_Date'] ? date('Y-m-d H:i:s', strtotime($row['Absence_Renew_Date'])) : null;
                ...
                if ($r->isSuccess()) {
                    ...
                    if ($real && $real['UF_CHANGED_AT']) {
                        ...
                        $u = "UPDATE ".GATE_DB_DBO.".".GATE_TABLE."
                              SET Absence_Renew_Date = N'".$sqlHelper->forSql($realTime)."'
                              WHERE Absence_ID = N'".$sqlHelper->forSql((string)$id)."'";
                        try { $gateConn->queryExecute($u); } catch (\Throwable $e) {
